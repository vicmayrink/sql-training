...
\begin{document}
	...
	\begin{frame}
		...
		\reasonboxbegin{Motivo 1}
			SQL é uma habilidade \textit{muito valorizada} no mercado
		\reasonboxend
		\reasonboxbegin{Motivo 2}
			Conhecer SQL é útil para profissionais de \textit{diversas especialidades}
		\reasonboxend
		\reasonboxbegin{Motivo 3}
			SQL é uma \textit{linguagem universal} para bancos de dados relacionais.
		\reasonboxend
	\end{frame}
	...
	\begin{frame}
		\frametitle{Por quê aprender SQL?}
		\reasonboxbegin{Motivo 4}
			O entendimento de como os dados da empresa estão organizados contribui para que os uma \textit{visão mais sistêmica do negócio} e dos processos operacionais.
		\reasonboxend
		\reasonboxbegin{Motivo 5}
			\textit{Individualmente}, o SQL contribuí para a capacidade analítica do profissional
		\reasonboxend
		\reasonboxbegin{Motivo 6}
	...
	\begin{frame}
		...
		Vai atualizar o valor da coluna \code{data\_hora\_consulta} para \texttt{2024-07-31 15:00} de \textbf{todos} os registros que satifazem as condições \texttt{nome\_paciente = `Maria Silva'} e \texttt{nome\_medico = `Dra. Costa'} e \texttt{data\_hora\_consulta = `2024-07-18 11:00:00'}.

		No nosso caso, havia somente 1 registro nessas condições.
	\end{frame}
