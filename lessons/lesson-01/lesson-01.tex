...
\begin{document}
	...
	\begin{frame}
		...
		\medskip
		\begin{figure}[h]
			\fbox{\includegraphics[height=42mm]{docker-hub-homepage.png}}
			\hspace{0.5cm}
			\fbox{\includegraphics[height=42mm]{docker-signup.png}}
		\end{figure}
	\end{frame}
	...
	\begin{frame}
		...
		\end{enumerate}
		\begin{figure}[h]
			\fbox{\includegraphics[height=38mm]{docker-desktop-download.png}}
			\hspace{0.5cm}
			\fbox{\includegraphics[height=38mm]{docker-installation.png}}
		\end{figure}
	\end{frame}
	...
	\begin{frame}
		...
		\begin{figure}[h]
			\fbox{\includegraphics[width=0.70\textwidth]{docker-window.png}}
		\end{figure}
	\end{frame}
	...
	\begin{frame}
		...
		Você será redirecionado para fazer o login pelo navegador.
		\begin{figure}[h]
			\fbox{\includegraphics[width=0.70\textwidth]{docker-desktop-sign-in.png}}
		\end{figure}
	\end{frame}

	%Frame: Docker Pulling an image
	\begin{frame}
		\frametitle{Passo 5: Baixar a imagem do MySQL}
		Na barra de pesquisas (na parte superior) digite "mysql".
		Procure pela imagem chamada "mysql" com uma tag indicando que trata-se de uma imagem oficial do Docker.
		
		\smallskip
		Clique no botão "Pull" 
		\begin{figure}[h]
			\fbox{\includegraphics[height=40mm]{docker-image-search.png}}
		\end{figure}
	\end{frame}
	...
	\begin{frame}
		\frametitle{Execução do container}
		\bigskip \bigskip
		\begin{itemize}
			\centering
			\Large
			\item[] Já baixamos a \textbf{\textit{imagem}} do MySQL.
			\bigskip \bigskip
			\item[] É hora de executar um \textbf{\textit{container}}.
		\end{itemize}
	\end{frame}

	%Frame: Docker - Image vs Container
	\begin{frame}
		\frametitle{Imagem \& Container}
		\begin{columns}[t]
			\begin{column}{0.40\textwidth}
				\begin{figure}[h]
					\fbox{\includegraphics[height=60mm]{image-vs-container.jpg}}
				\end{figure}
			\end{column}
			\begin{column}{0.55\textwidth}
				\bigskip \bigskip
				\reasonboxbegin{Imagem x Container}{MaterialTeal800}
	...
	\begin{frame}
		...
		\begin{columns}[t]
			\begin{column}{0.40\textwidth}
				\begin{figure}[h]
					\fbox{\includegraphics[height=60mm]{image-vs-container-confident.jpg}}
				\end{figure}
			\end{column}
			\begin{column}{0.55\textwidth}
				\bigskip

				O container é uma \textbf{instância} de execução de uma imagem.
				\bigskip
				\begin{itemize}
					\item[] \textbf{Imagem:} é como uma forma de fazer bolos
					\bigskip
					\item[] \textbf{Container:} é como um bolo que foi produzido a partir de uma forma
				\end{itemize}
			\end{column}
			\begin{column}{0.05\textwidth}
			\end{column}
	...
	%Frame: Docker - Image vs Container
	\begin{frame}
		\frametitle{Executando um container do MySQL}
		\begin{enumerate}
			\item No menu lateral do Docker, clique em "\textit{Images}".
			\item Localize a imagem do MySQL que acabamos de baixar.
			\item Clique no botão $\filledmedtriangleright$ para executar um container a partir desta imagem.
		\end{enumerate}
		\begin{figure}[h]
			\fbox{\includegraphics[height=50mm]{run-container.png}}
		\end{figure}
	\end{frame}

	\begin{frame}[t]
		\frametitle{Passo 5: Baixar a imagem do MySQL}
		Vamos baixar também a imagem de um banco MySQL que já possui dados pré-carregados.
		
		Repita o procedimento, desta vez vamos baixar a imagem \textbf{\texttt{sakiladb/mysql}}
		\bigskip
		\begin{figure}[h]
			\fbox{\includegraphics[width=0.45\textwidth]{docker-sakila-search-1.png}}
			\hspace{0.5cm}
			\fbox{\includegraphics[width=0.45\textwidth]{docker-sakila-search-2.png}}
		\end{figure}
	\end{frame}
